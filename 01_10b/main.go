...
func parseOperand(op string) (*float64,error) {
	...
	if err!= nil{
		return nil,fmt.Errorf("cannot parse operand %v", err)
	}
	return &parsedOp,nil
}
...
func calculate(expr string) (*float64,error) {
	...
	nops:=len(ops)
	if nops != 3{
		return nil,fmt.Errorf("invalid number of ops need 3 got %d",nops)
	}
	...
	if !ok {
		return nil,fmt.Errorf("cannot calculate:%v is unknown",ops[1])
	}
	result := f(*left, *right)
	return &result,nil
}
